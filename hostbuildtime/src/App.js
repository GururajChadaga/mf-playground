...
console.log("hostbuildtime sharescope", __webpack_share_scopes__);
    ...
    >
      ...
      <div style={{ display: "flex", gap: "1em", marginBottom: "0.5em" }}>
        ...
        <Link
          ...
          style={{
            textDecoration:
              location.pathname === "/demos" ? "underline" : "none",
          }}
        >
          Demos
        </Link>
        <Link
          ...
          style={{
            textDecoration:
              location.pathname === "/remotes" ? "underline" : "none",
          }}
        >
          Remotes
        </Link>
        ...
        <div>
          ...
          {appStats && (
            ...
            >
              ...
              <p>
                <strong>Active Apps:</strong> {appStats.activeApps}
              </p>
              <p>
                <strong>Last Updated:</strong>{" "}
                {new Date(appStats.lastUpdated).toLocaleTimeString()}
              </p>
            </div>
          )}
        </div>

        <div>{/* Empty cell for 2x2 grid */}</div>
      </div>
    </div>
  );
}
...
function RemotesPage() {
  ...
  const {
    ...
    loading,
    error,
  } = useDynamicRemote({ scope, module });

  return (
    <div>
      <h2>Remote Applications</h2>
      <p>
        Load and test remote Module Federation applications using dynamic
        loadRemote:
      </p>
      ...
      <div style={{ marginTop: "2em" }}>
        ...
        {error && (
          <p style={{ color: "red" }}>Error loading remote: {error.message}</p>
        )}
